...
function setAvailability(available, props) {
  ...
  if(available) {
    axios.post(props.availabilityEndpoint, { available: true })
    props.setupDevice(props.token)
    props.updateAvailability(true)
  } else {
    axios.post(props.availabilityEndpoint, { available: false })
    props.updateAvailability(false)
    props.destroyDevice()
  }
